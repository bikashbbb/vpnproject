    ...
    func configureVPN(config: String?, username : String?,password : String?,completion:@escaping (_ error : Error?) -> Void) {
        ...
        self.providerManager?.loadFromPreferences { error in
            if error == nil {
                ...
                tunnelProtocol.providerConfiguration = ["config": configData?.data(using: .utf8) ?? "","username" : username?.data(using: .utf8) ?? "","password" : password?.data(using: .utf8) ?? ""]
